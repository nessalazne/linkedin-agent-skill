...
**5. This skill never publishes.** It produces text. On "yes", append the
post to `~/.claude/linkedin/log.md` with the date, the hook used and the first
line, so `/li-audit` has a history to work from later. If the user wants it
posted or scheduled, run `/li-publish`, which is the only skill in the pack
that touches LinkedIn, and it asks again before it does.
